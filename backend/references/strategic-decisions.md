...
|---|---|---|---|---|
| 1 | **Generate a short story** | 1 | Includes cover image and audio narration | `generation` |
| 2 | **Continue a story (author writes a new chapter)** | 1 per chapter | Author-only. Includes chapter cover art. | `generation` |
...
Rewarded ad = 1 credit / 24 hours. As covered in the Blueprint, this **loses money per ad-funded generation** but it's the mechanism that builds the daily-open habit, which in turn drives paid conversion (target: 3-8% of free users).
...
```sql
-- Add reader_earning to allowed reasons
-- (Blueprint's credit_ledger.reason column just needs its enum/check constraint expanded.)
--   Existing: 'purchase', 'subscription', 'ad_reward', 'streak',
