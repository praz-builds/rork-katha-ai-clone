...
b_start = balance(uid)
if b_start != 60:
    print(f"  unexpected starting balance {b_start}"); sys.exit(1)
...
ch_a = None
if s != 200:
    print("  body:", json.dumps(d)[:500]); FAIL.append("2: standalone generation")
else:
...
series_id = None
if s != 200:
    print("  body:", json.dumps(d)[:500]); FAIL.append("4: series generation")
else:
    st, ch = d["story"], d["chapter"]
    series_id = st["id"]; story_ids.append(series_id)
...
if series_id:
    ...
    print(f"  HTTP {s}")
    if s != 200:
        print("  body:", json.dumps(d)[:500]); FAIL.append("5: mid-series continuation")
    else:
        ch2 = d["chapter"]
        ...
        check("5.5 series_state advanced from ch1", new_state != results.get("ch1_state"),
              "changed" if new_state != results.get("ch1_state") else "IDENTICAL - not advancing")
...
if series_id:
    ...
    print(f"  HTTP {s}")
    if s != 200:
        print("  body:", json.dumps(d)[:500]); FAIL.append("6: finale continuation")
    else:
        ch3 = d["chapter"]
...
print(f"  HTTP {s}")
if s != 200:
    print("  body:", json.dumps(d)[:500]); FAIL.append("7: kids series generation")
else:
    st, ch = d["story"], d["chapter"]
    story_ids.append(st["id"])
