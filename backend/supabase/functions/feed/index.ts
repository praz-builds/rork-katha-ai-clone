...
serve(async (req) => {
  ...
  try {
    ...
    // Build continue_reading list (stories user started but have unread chapters)
...
async function buildNewUserFeed(
  ...
  // Adjust offset for the fill query: if past all curated, offset into public
...
async function buildReturningUserFeed(
  ...
  // Fetch user's followed authors and already-read story IDs in parallel
  ...
  // Fetch a larger batch to score and sort in-memory, then paginate.
  ...
  for (const story of candidates ?? []) {
    ...
    if (storyGenres.some((g) => preferredGenreSet.has(g.toLowerCase()))) {
      score += 3;
    }
...
async function buildContinueReading(
  ...
  // by comparing total published chapters vs user's read count for that story.
  ...
  for (const story of stories) {
    ...
    // If more published chapters than user read entries, suggest continuing
