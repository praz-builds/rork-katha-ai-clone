...
Deno.test("the owner's own profile needs a session; a guest is refused", async () => {
  for (
    const action of ["me", "username", "bio", "avatar", "preferences", "set_preferences"]
  ) {
    const response = await handleProfile(
      new Request("https://example.com/profile", {
        method: "POST",
...
// ---------------------------------------------------------------------------
// Global preferences: languages and home (00100)
// ---------------------------------------------------------------------------

/** One preferences request against a stubbed network, recording the RPC body. */
async function preferencesRequest(
  body: Record<string, unknown>,
  rpcAnswer: unknown = { saved: true },
): Promise<{
  status: number;
  body: Record<string, unknown>;
  ...
  globalThis.fetch = (async (
    ...
    if (request.url.includes("/rpc/set_reader_preferences")) {
      rpcBodies.push(await request.json());
      return json(rpcAnswer);
    }
    if (request.url.includes("/rest/v1/reader_preferences")) {
      return json({ spoken_languages: ["hi", "en"], home_place: "Pune" });
    }
    return json({ message: `unexpected ${request.url}` }, 404);
  }) as typeof fetch;
  ...
  try {
    ...
    return { status: response.status, body: await response.json(), rpcBodies, urls };
  } finally {
    globalThis.fetch = originalFetch;
    for (const [key, value] of Object.entries(previous)) {
      if (value === undefined) Deno.env.delete(key);
...
Deno.test("preferences are saved for the token's user, never the body's", async () => {
  const { status, body, rpcBodies } = await preferencesRequest({
    action: "set_preferences",
    // A body that names somebody else is ignored: the id is the token's.
    userId: PROFILE_AUTHOR,
    spokenLanguages: ["hi", "en"],
    homePlace: "  Pune ",
  });
  assertEquals(status, 200);
  assertEquals(body, { spokenLanguages: ["hi", "en"], homePlace: "Pune" });
  assertEquals(rpcBodies, [{
    p_user_id: PROFILE_VIEWER,
    p_spoken_languages: ["hi", "en"],
    p_home_place: "Pune",
  }]);
});
...
Deno.test("preferences are read back for the token's user", async () => {
  const { status, body, urls } = await preferencesRequest({
    action: "preferences",
  });
  assertEquals(status, 200);
  assertEquals(body, { spokenLanguages: ["hi", "en"], homePlace: "Pune" });
  const read = urls.find((url) => url.includes("/rest/v1/reader_preferences"));
  assert(read?.includes(`user_id=eq.${PROFILE_VIEWER}`));
});
