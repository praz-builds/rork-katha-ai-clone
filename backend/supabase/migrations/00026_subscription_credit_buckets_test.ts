...
Deno.test("subscription refresh replaces only the grant bucket", async () => {
  ...
  try {
    ...
    await db.query(
      "select grant_credit($1, 10, 'purchase', 'pack', 'rc:pack')",
      [userId],
    );
    await db.query(
      "select refresh_subscription_grant($1, 20, 'month-1', 'rc:month-1')",
      [userId],
    );
    await db.query(
      "select deduct_credit($1, 5, 'generation', 'story', 'generation:1')",
      [userId],
    );
    const refreshed = await db.query<{ refresh_subscription_grant: number }>(
      "select refresh_subscription_grant($1, 20, 'month-2', 'rc:month-2')",
      [userId],
    );
    ...
    >(
      "select subscription_grant_balance, purchased_balance, earned_balance from credit_balance_buckets where user_id = $1",
      [userId],
    );
    assertEquals(buckets.rows[0], {
      subscription_grant_balance: 20,
...
Deno.test("chargebacks clamp and record the shortfall without a negative balance", async () => {
  ...
  try {
    ...
    const balance = await db.query<{ deduct_credit: number }>(
      "select deduct_credit($1, 40, 'chargeback', 'refund', 'rc:refund')",
      [userId],
    );
    assertEquals(balance.rows[0].deduct_credit, 0);
    const chargeback = await db.query<
      { debited_amount: number; shortfall_amount: number }
    >(
      "select debited_amount, shortfall_amount from credit_chargebacks where user_id = $1",
      [userId],
    );
    assertEquals(chargeback.rows[0], {
      debited_amount: 10,
      shortfall_amount: 30,
    });
  } finally {
...
Deno.test("automatic refunds restore the bucket that funded the generation", async () => {
  ...
  try {
    ...
    await db.query("insert into profiles(id) values ($1)", [userId]);
    await db.query(
      "select refresh_subscription_grant($1, 20, 'month', 'rc:month')",
      [userId],
    );
    await db.query(
      "select deduct_credit($1, 5, 'generation', 'story', 'generation:story')",
      [userId],
    );
    await db.query(
      "select grant_credit($1, 5, 'refund', 'story', 'refund:story')",
      [userId],
    );
    const buckets = await db.query<
      {
...
Deno.test("expiration writes one negative lapse ledger entry and is idempotent", async () => {
  ...
  try {
    ...
    await db.query("select lapse_credits($1, 'expired', 'rc:expired')", [
      userId,
    ]);
    await db.query("select lapse_credits($1, 'expired', 'rc:expired')", [
      userId,
...
Deno.test("partial automatic refunds restore only the refunded allocation", async () => {
  ...
  try {
    ...
    await db.query("insert into profiles(id) values ($1)", [userId]);
    await db.query(
      "select refresh_subscription_grant($1, 5, 'month', 'rc:month')",
      [userId],
    );
    await db.query(
      "select deduct_credit($1, 5, 'generation', 'story', 'generation:story')",
    ...
    await db.query(
      "select grant_credit($1, 2, 'refund', 'story', 'refund:story')",
      [userId],
    );
    const buckets = await db.query<
      {
