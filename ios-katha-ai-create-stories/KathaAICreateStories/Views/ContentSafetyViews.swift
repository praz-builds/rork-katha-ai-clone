...
struct AppLanguageSelectionSheet: View {
    ...
    var body: some View {
        ZStack {
            SafetySheet(height: 390) {
                VStack(alignment: .leading, spacing: KathaTheme.Spacing.m) {
                    HStack {
                        Text(safetyText("language.app.title"))
                            .font(.system(size: 24, weight: .bold))
                            .foregroundStyle(KathaTheme.textPrimary)
                        Spacer()
                        Button { appState.showUILanguageSheet = false } label: {
                    ...
                    Text(safetyText("language.app.subtitle"))
                        .font(.system(size: 14))
                        .foregroundStyle(KathaTheme.textSecondary)
                        .padding(.horizontal, KathaTheme.Spacing.xl)

                    VStack(spacing: 0) {
                        languageRow(flag: "🇺🇸", title: "English", native: "English", selected: true) {
                            appState.uiLanguage = .english
                        }
                        Divider().background(KathaTheme.border)
                        languageRow(flag: "🇮🇳", title: "Hindi", native: "हिन्दी", selected: false) {
                            showComingSoon = true
                        }
                    }
            ...
            if showComingSoon {
                ...
                VStack(spacing: KathaTheme.Spacing.m) {
                    Image(systemName: "character.book.closed")
                        ...
                        .background(Circle().fill(KathaTheme.accentSoft))
                    Text(safetyText("language.hindi.title"))
                        .font(.system(size: 20, weight: .bold))
                        .foregroundStyle(KathaTheme.textPrimary)
                        .multilineTextAlignment(.center)
                    Text(safetyText("language.hindi.subtitle"))
                        .font(.system(size: 14))
                        .foregroundStyle(KathaTheme.textSecondary)
                        .multilineTextAlignment(.center)
                    VStack(spacing: KathaTheme.Spacing.m) {
                        PrimaryCTA(title: safetyText("language.hindi.notify")) {
                            appState.notifyHindiAvailability()
                            showComingSoon = false
                        }
                        SecondaryCTA(title: safetyText("language.hindi.continue")) {
                            showComingSoon = false
                        }
                    }
                }
        ...
        .animation(.spring(response: 0.3, dampingFraction: 0.8), value: showComingSoon)
    }

    private func languageRow(flag: String, title: String, native: String, selected: Bool, action: @escaping () -> Void) -> some View {
        Button(action: { Haptics.light(); action() }) {
            HStack(spacing: KathaTheme.Spacing.m) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(native).font(.system(size: 13)).foregroundStyle(KathaTheme.textSecondary)
                }
                Spacer()
                if selected {
                    Image(systemName: "checkmark")
                        ...
                        .background(Circle().fill(KathaTheme.accent))
                } else {
                    Circle().stroke(KathaTheme.borderStrong, lineWidth: 1.5).frame(width: 18, height: 18)
                }
            }
            .padding(.horizontal, KathaTheme.Spacing.xl)
            .frame(height: 62)
            .opacity(title == "Hindi" ? 0.6 : 1)
        }
        .buttonStyle(.plain)
    }
}
