...
The dev Mac (2017 Intel MacBook Pro, macOS 13.7 Ventura) has a full local
Android toolchain, installed machine-wide in the home folder on 2026-09-14:
...
| AVD | `Pixel_7_API_36` (Android 16, x86_64 Google APIs image, Hypervisor.framework acceleration) |
| Env vars | `~/.android-env.sh`, sourced by `~/.bash_profile` and `~/.zshrc` |

```bash
emulator -avd Pixel_7_API_36 &        # boot the phone (about 60s)
cd expo && pnpm android                # = expo run:android: prebuild, Gradle build, install, start Metro
pnpm start --dev-client                # Metro only, when the dev client is already installed
```
...
- In a non-login shell (agents, scripts), run `. ~/.android-env.sh` before `adb`, `emulator`, or Gradle.
- The first Gradle build takes 10-20 minutes on this CPU; later builds are incremental.
