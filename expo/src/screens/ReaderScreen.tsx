...
export type ReaderScreenProps = {
   ...
   *
   * THE HOOK POINT FOR THE REIMAGINE SHEET. `ReaderChrome` already renders a
   * Reimagine control whenever this is supplied, for every reader and not only
   * the author (a reader of someone else's story gets a private copy). The
   * agent building `ReimagineSheet` needs exactly two edits in this file: pass
   * a handler that flips one piece of state, and render the sheet at the
   * marked block beside `MusicPicker` at the bottom of this component.
   */
  onReimagine?: () => void;
};
    ...
    <View style={[styles.reader, { backgroundColor: theme.background }]}>
      ...
      */}
      ...
      <ReaderChrome
        ...
        // private copy - and is likewise offered only once there is a whole
        // chapter to reimagine. Both are ABSENT rather than disabled before
        // that: a greyed control mid-generation is a question the writer
        // cannot answer.
        onEdit={isAuthor && chapterComplete ? () => setEditOpen(true) : undefined}
        onReimagine={onReimagine && chapterComplete ? onReimagine : undefined}
